...
3. Declare `[[panes]] id = "issues"` with `placement = "split"`; the user opens it via `herdr plugin pane open`, a keybinding → `plugin_action`, or a `[[link_handlers]]` match on GitHub issue URLs. **[verified] A `plugin_action` keybinding resolves an action id only** — binding the pane entrypoint id answers `plugin_action_not_found`, and `herdr plugin action list` never lists panes. So a bindable pane needs a one-line `[[actions]]` entry whose command is `herdr plugin pane open --plugin "$HERDR_PLUGIN_ID" --entrypoint <pane id>`. Also verified: `[[keys.command]]` takes `key`, `type`, `command` and `description`, and `herdr config check` rejects anything else (`plugin`/`action` are unknown keys).
